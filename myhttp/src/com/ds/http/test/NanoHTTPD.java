    ...
    public Response serve(String uri, String method, Properties header, Properties parms, Properties files) {
        ...
        while (e.hasMoreElements()) {
            ...
            if(value.equals("Filename")){
                fileName=parms.getProperty(value);
            }
/*            if(value.equals("Filename")){
                File upFile=new File("/sdcard/.ds",parms.getProperty(value));
                try {
        ...
        while (e.hasMoreElements()) {
            ...
            if(value.equals("Filedata")){
                File temFile=new File(files.getProperty(value));
                if(temFile.exists()&&uploadUrl!=null){
                    //File upload=new File("/sdcard/.ds/upload");
                    File upload=new File(uploadUrl);
                    if(!upload.exists()){
                        upload.mkdirs();
                    }
                    //temFile.renameTo(new File("/sdcard/.ds/upload",fileName));
                    temFile.renameTo(new File(uploadUrl,fileName));
                    temFile.delete();
                }
            }
    ...
    private class HTTPSession implements Runnable {
        ...
        private String saveTmpFile(byte[] b, int offset, int len) {
            ...
            if (len > 0) {
                ...
                try {
                    File temp = File.createTempFile("file", "", new File(tmpdir));
    ...
    public Response serveFile(String uri, Properties header, File homeDir,
        ...
        if (res == null && f.isDirectory()) {
            ...
            if(filess!=null){
            File[] sortFiles=new File[filess.length];
            for(int k=0;k<filess.length;k++){
                String fs=encodeUri(uri+filess[k]);
                sortFiles[k]=new File(fs);
            }
            if(sortFiles!=null){
                sort(sortFiles);
            }
            String[] files=new String[sortFiles.length];
            for(int j=0;j<files.length;j++){
                int start=sortFiles[j].toString().lastIndexOf("/")+1;
                files[j]=sortFiles[j].toString().substring(start);
            }
            ...
            if (res == null) {
                ...
                if (new File(f, "index.html").exists())
                    ...
                    //String[] files = f.list();
                    uploadUrl=uri;
                    String msg = "<html><body><h1>目录 " + uri + "</h1><br/>"+"<br/>"+"<html xmlns=\"http://www.w3.org/1999/xhtml\" >\n" +
                            "<head>\n" +
                            "    <meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\" />\n" +
                            "<link href=\"/sdcard/.ds/default.css\" rel=\"stylesheet\" type=\"text/css\" />"+
                            "    <title>点石矩阵</title>\n" +
                            "    <script type=\"text/javascript\" src=\"/sdcard/.ds/swfupload.js\"></script>\n" +
                            "    <script type=\"text/javascript\" src=\"/sdcard/.ds/swfupload.queue.js\"></script>\n" +
                            "    <script type=\"text/javascript\" src=\"/sdcard/.ds/fileprogress.js\"></script>\n" +
                            "    <script type=\"text/javascript\" src=\"/sdcard/.ds/handlers.js\"></script>\n" +
                            "    <script type=\"text/javascript\">\n" +
                            "        var swfu;\n" +
                            "        window.onload = function() {\n" +
                            "            var settings = {\n" +
                            "                flash_url : \"/sdcard/.ds/swfupload.swf\",\n" +
                            "                upload_url: \"/sdcard/.ds/index_.html\",\n" +
                            ...
                            "                debug: false,\n" +
                            "                // Button settings\n" +
                            "                button_image_url: \"/sdcard/.ds/TestImageNoText_65x29.png\",\n" +
                            "                button_width: \"65\",\n" +
                            "                button_height: \"29\",\n" +
                    ...
                    if (uri.length() > 1) {
                        ...
                        if (slash >= 0 && slash < u.length())
                            msg += "<b><a href=\"" + uri.substring(0, slash + 1) + "\">上一级目录</a></b><br/>";
                    }
                    ...
                    if (files != null) {
                        for (int i = 0; i < files.length; ++i) {
                            ...
                            File temFile=new File(encodeUri(uri + files[i]));
                            if(temFile.isDirectory()){
                                msg +="<div class=\"dhx_dataview_item dhx_dataview_ficon_item\"\n" +
                                        "     style=\"padding:10px;margin:10px;float:left; overflow:hidden;\">\n" +
                                        "    <div align=\"center\"><a href="+encodeUri(uri+files[i])+"><img src=\"/sdcard/.ds/1.jpg\" width=\"100px\"  height=\"100px\"\"></a>\n" +
                                        "        <div class=\"dhx_item_text\"><a href="+encodeUri(uri+files[i])+">"+files[i]+"</a></div>\n" +
                                        "    </div>\n" +
                                        "</div>";
                            }else{
                                msg +=  "<div class=\"dhx_dataview_item dhx_dataview_ficon_item\"\n" +
                                        "     style=\"padding:10px;margin:10px;float:left; overflow:hidden;\">\n" +
                                        "    <div align=\"center\"><a href="+encodeUri(uri+files[i])+"><img src=\"/sdcard/.ds/2.jpg\" width=\"100px\"  height=\"100px\"\"></a>\n" +
                                        "        <div class=\"dhx_item_text\"><a href="+encodeUri(uri+files[i])+">"+files[i]+"</a></div>\n" +
                                        "    </div>\n" +
                                        "</div>";
                            }
...
/*                            if (curFile.isFile()) {
    ...
    private static int theBufferSize = 1024 * 1024;
